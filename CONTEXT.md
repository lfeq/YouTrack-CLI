...
**Subtask link**:
A directed parent/child relationship between two **Issues** (`parent for` on the Epic, `subtask of` on the child). It is how implementation issues attach to their PRD Epic. One of two link types the CLI manages, alongside the **Dependency link**. Surfaced for reading in `issue show` (and `--json` as `parent`/`subtasks`) so callers can verify the hierarchy programmatically; see ADR 0007.
...
- **Output encoding**: All human-readable output is forced to UTF-8. At startup the CLI reconfigures `sys.stdout`/`sys.stderr` to `encoding="utf-8", errors="backslashreplace"` (via a guarded helper in `console.py`, called from `main()`), unconditionally — no `isatty()` gate, so console, pipe, and file output share one deterministic encoding on every OS. This prevents the legacy-Windows cp1252 `UnicodeEncodeError` that otherwise crashes any table path (`issue show`/`list`, `project list`, `tag list`) on a non-cp1252 glyph (e.g. `→`); `--json` was already safe because `json.dumps` defaults to `ensure_ascii=True`. `backslashreplace` guarantees a stray glyph degrades to an escape rather than crashing. See `docs/adr/0006`.
...
- **PRD workflow**: A PRD is published as an Epic issue (`issue create --type Epic --description <prd>`). Implementation issues are linked beneath it with `--parent <epic-id>` (at creation) or `issue link` (after the fact), forming a subtask hierarchy. Progress is recorded with `issue comment` (append-only, the safe default); the PRD body itself is revised with `issue update`. `issue show` returns the Epic's body, all comments, and its `parent`/`subtasks` (each with resolved status) together so an agent can resume work — and read back the hierarchy it created with `--parent` — from a single call. `--json` exposes `parent` (single object or null) and `subtasks` (array) using the `{id, summary, resolved}` shape. See `docs/adr/0001`–`0003`, `0007`.
